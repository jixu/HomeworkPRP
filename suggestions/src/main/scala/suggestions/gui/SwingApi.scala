...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = {
      Observable { observer: Observer[String] =>
        val r: Reaction = {
          case e: Event => ValueChanged.unapply(e) match {
            case Some(t) => observer.onNext(t.text)
            case None => None
          }
        }
        field.subscribe(r)
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = {
      Observable { observer: Observer[Button] =>
        val r: Reaction = {
          case e: Event => ButtonClicked.unapply(e) match {
